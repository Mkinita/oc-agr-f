...
const FormularioRema = () => {
    const [tipo, setTipo] = useState ('REMA')
    ...
    const {guardarRema, rema} = useRema()
    ...
    useEffect(() => {
        if(rema?.tipo){
            setTipo(rema.tipo)
            setCliente(rema.cliente)
            setNumeroOc(rema.numeroOc)
            setProducto(rema.producto)
            setSuma(rema.suma)
            setNumero1(rema.numero1)
            setId(rema._id)
        }
    },[rema])
    ...
    const handleSubmit =  e => {
        ...
        setAlerta({})
        guardarRema({tipo,cliente,numeroOc,producto,suma,numero1,id})
        setAlerta({
  ...
  return (
    <>
        ...
        <h3 className='font-black text-3xl text-center'>REMA</h3>
