...
const FormularioCol = () => {
    const [tipo, setTipo] = useState ('COL')
    ...
    useEffect(() => {
        if(col?.tipo){
            setTipo(col.tipo)
            setCliente(col.cliente)
            setNumeroOc(col.numeroOc)
            setProducto(col.producto)
            setSuma(col.suma)
            setNumero1(col.numero1)
            setId(col._id)
        }
    },[col])
    ...
    const handleSubmit =  e => {
        ...
        setAlerta({})
        guardarCol({tipo,cliente,numeroOc,producto,suma,numero1,id})
        setAlerta({
  ...
  return (
    <>
        ...
        <h3 className='font-black text-3xl text-center'>COL</h3>
